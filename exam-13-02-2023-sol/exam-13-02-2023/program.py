...
def reversedic(D):
    return {k:sorted(v, reverse=True) for k,v in D.items()}
...
def func2(list_all, list_rm):
    ...
    while i < len(list_all):
        if list_all.count(list_all[i]) > 1:
            del list_all[i]
        if list_all[i] not in list_rm:
            del list_all[i]
        else : i+=1
...
def func4(M):
    ...
    return onrows(M) + oncols(M)

def onrows(M):
    onrows = []
    R = 1
    for row in M:
        onrows.append(sum(row))
    for num in onrows:
        R *= num
...
def oncols(M):
    ...
    while len(oncols) < len(M[0]):
        for row in M:
            tmp.append(row[i])
        oncols.append(sum(tmp))
        tmp.clear()
        i += 1
...
def func5(points):
    ...
    x,y = 0,0
    list_dist = compute_distances(points)
    
    for point in list_dist:
        x += point[0][0]
        y += point[0][1]
...
def compute_distances(points):
    distances = {point : round(((point[0])**2 + (point[1])**2)**0.5, 3) for point in points}
    list_dist = list(distances.items())
    list_dist.sort(key= lambda x: x[1])
    return list_dist[:3]
# bastava anche sortare i punti cristoddio

# %% ----------------------------------- EX.1 ----------------------------------- #
'''
Epoint[0]: 6 points
    Implement the epoint[0](root, result) function, recursive or using
...
def traversal(dirin, words, dic):
    for el in os.listdir(dirin):
        if os.path.isdir(dirin + "/" + el):
            traversal(dirin + "/" + el, words, dic)
            
        elif os.path.splitext(dirin + "/" + el)[1] == '.txt':
            dic.update(makedict(dirin + "/" + el , words, dic))
            
    return list(dic.items())
            
def makedict(path, words, d):
    with open(path) as f:
        txt = f.read().split()
