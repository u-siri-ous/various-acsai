...
def func1(string_list1, string_list2):
    ...
    for st in string_list1:
        if st not in string_list2 and st not in ret:
            ret.append(st)
            
    for st in string_list2:
        if st not in string_list1 and st not in ret:
            ret.append(st)
...
def func3(a_list):
    ...
    while i < len(a_list):
        if a_list[i] == max_l or a_list[i] == min_l:
            del a_list[i]
        else:
            i += 1
    
    return initial - len(a_list)
...
def makerows(lst):
    ...
    while lst:
        ...
        count += 1
        ch = lst[0][0].lower()
        while(lst and lst[0][0].lower() == ch):
            row.add(lst.pop(0))
    ...
    for i in range(len(rows)):
        rows[i].sort()
        rows[i].sort(key = lambda x: x.lower())
        rows[i].sort(key = len)
...
def func5(imagefile, output_imagefile, color):
    ...
    for k in range(len(image)):
        for i in range(len(image[k])):
            if image[k][i] == (255, 255, 255):
                break
            
        for j in range(len(image[k])-1, -1, -1):
            if image[k][j] == (255, 255, 255) or j < i:
                break
    ...
    images.save(image, output_imagefile)
    return count
...
def ex1(directory, ext):
    ...
    dic = {}
    path = postorder(directory, ext, dic)
    return path

def postorder(directory, ext, dic):
    
    for fol in os.listdir(directory):
        if os.path.isdir(directory + "/" + fol):
            postorder(directory + "/" + fol, ext, dic)
            
        elif fol.endswith(ext):
            if dic.get(directory):
                dic[directory] += os.path.getsize(directory + "/" + fol) 
            else:
                dic[directory] = os.path.getsize(directory + "/" + fol)     
...
def ex2(root):
    ...
    sums = createandsum(root, 0, 0)
    return sums

def createandsum(root, level, sums):
    ...
    sums = createandsum(root.left,  level+1, sums)
    sums = createandsum(root.right, level+1, sums)
