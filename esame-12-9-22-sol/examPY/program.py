...
def ex1(triangles):
    ...
    i = 0
    removed = 0
    while(i < len(triangles)): #modifica la lista direttamente (in-place), mentre la scorro
        (c1, c2, I) = sorted(triangles[i])
        if check((c1, c2, I)):
            i = i + 1
        else:
            triangles.pop(i)
...
def check(tup):
        return round((tup[0] ** 2 + tup[1] ** 2), 3) == round(tup[2] ** 2,3)
...
def aux(strings, n):
    ...
    if n == 1:
        ...
        return [ s+l for s in strings for l in aux(strings-{s}, n-1)]
    
def ex4(strings,n):
    return sorted(set(aux(strings, n)), key = lambda x : (-len(x),x))
