...
def func2(pathname):
    ...
    for line in txt:
        ...
        dic[couple[1]] = max(dic.get(couple[1]) or 0, couple[0])
        
        #value or 0 == value if true, 0 if false
        
        #if dic.get(couple[1]):
        #    dic[couple[1]] = max(couple[0], dic[couple[1]])
        #else: dic[couple[1]] = couple[0]
...
def func3(listaA, pathname):
    listaA = sorted(sorted(listaA, reverse = True), key = len)
...
#     return len(listaA) - listaA.count('\n')
...
def func4(S):
    ...
    lst = ''.join([ch if ch.isalpha() else ' ' for ch in S]).lower().split()
    lst = sorted(lst)
    
    long = len(max(lst, key=len)) + 1
    ...
    for key in dic.keys(): #for key in dic
        ret += key + ' ' * (long-len(key)) + dic[key] + '\n' 
...
un'immagine modellata come lista di liste e effettui una rotazione A DESTRA
...
def func5(image, output_file_name):
    
    rows = len(image)
    cols = len(image[0])
    
    for i in range(rows // 2):
        image[i], image[rows-i-1] = image[rows-i-1], image[i]
        
    # mat = [[(0,0,0) for j in range(rows)] for i in range(cols)]    
    # for i in range(rows):
    #     for j in range(cols):
    #         mat[j][i] = image[i][j]
            
    mat = [[image[i][j] for i in range(rows)] for j in range(cols)]
        
    images.save(mat, output_file_name)
    return (cols, rows)
...
def postorder(root, dic):
    ...
    for fol in os.listdir(root):
        ...
        if os.path.isdir(root + "/" + fol):
            postorder(root + "/" + fol, dic)
            
        elif fol.endswith(".txt"):
            files.append(root + "/" + fol)
...
def ex2(nums, ops):
    s = ''
    comb = set()
    count = 2*len(nums) - 1 #n + n - 1 elements per combo
    rec(nums, ops, s, comb, count)
    
    return comb
    
def rec(nums, ops, s, comb, count):  
    if count == 0:
        comb.add(s)
        return
    
    if count%2: #needs number
        for num in nums:
            nn = nums - {num}
            rec(nn, ops, s + str(num), comb, count - 1)
        
    else: #needs operation
        for op in ops:
            rec(nums, ops, s + op, comb, count - 1)
