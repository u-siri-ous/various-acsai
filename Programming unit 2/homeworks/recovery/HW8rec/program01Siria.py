...
def control_one_color(lista,color): #verifico che la linea orizzontale un solo colore
    for i in lista:
        if i!=color:
            return False
    else:
...
def processImage(matrix, color, colorlist): #startimage 
    ...
    for el, row in enumerate(matrix):
        if control_one_color(row, row[0]):
            if row[0] == color:
                return colorlist
            else:    
                colorlist.append(row[0])
                for v in range(len(row)):
                    if matrix[el-1][v]!= row[0]:
                        continue
                    else:
                        break
...
# 
#     matrix = images.load(input_file)
#     colorlist = processImage(matrix, matrix[0][0], [matrix[0][0]])
#     images.save([colorlist], output_file)
#     sections = (((len(colorlist)-1)*3)+1)
#     return sections
#      
# if __name__ == '__main__':
#     #ex1("./puzzles/fullcase01.in.png", "d.png")
#     pass
# =============================================================================