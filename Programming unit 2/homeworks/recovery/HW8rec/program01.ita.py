...
def readProcess(input_file):
    matrix = images.load(input_file) #works, returns matrix
    return matrix

def processImage(matrix, bg):
    image = []
    ...
    for x in matrix:
        for y in x:
            if all(matrix):
                return matrix
            image.append(x[0])
            vert = matrix[0].index(x[0])
            
            image = processImage([sec[x+1:] for sec in matrix[y+1:]], bg)
            image = processImage([sec[:x] for sec in matrix[y+1:]], bg)
            image = processImage([sec[x+1:] for sec in matrix[:y]], bg)
            image = processImage([sec[:x] for sec in matrix[:y]], bg)
    
    return image

def ex1(input_file,  output_file):
    matrix = readProcess(input_file)
    background_clr = matrix[0][0]
    
    image = (matrix, background_clr)
    
    images.save([matrix], output_file)
    
    return((len(image)-1)*3)+1
    return 

if __name__ == '__main__':
    #readfile(f"puzzles/small01.in.png")
    pass
